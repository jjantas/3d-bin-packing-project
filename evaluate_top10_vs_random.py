...
def pick_top_cfgs(summary_csv: str, top_n: int = 10) -> pd.DataFrame:
    ...
    params_cols = [
        "elitism", "fitness_mode", "generations",
        "init_constructive_ratio", "init_strategy",
        "mutation_strength", "p_crossover",
        "p_mut_move", "p_mut_presence", "p_mut_resupport", "p_mut_rot",
        "pop_size", "prob_presence_init", "ratio_to_remove",
        "report_mode", "selection", "threshold_keep_ratio", "tournament_k",
    ]

    first_rows = df.sort_values(["cfg_id"]).drop_duplicates("cfg_id")
    top = top.merge(first_rows[["cfg_id"] + [c for c in params_cols if c in first_rows.columns]],
                    on="cfg_id", how="left")
...
# -------------------------
def main():
    ap = argparse.ArgumentParser(description="Evaluate TOP-10 GA configs vs random_search on 3 warehouse sizes.")
    ap.add_argument("--summary_csv", type=str, required=True, help="CSV z podsumowaniem eksperymentów (z score_from_conv).")
    ap.add_argument("--boxes_csv", type=str, required=True, help="CSV z pudełkami (l,w,h) – użyte w ewaluacji.")
    ap.add_argument("--out_csv", type=str, default="runs/eval_top10_vs_random/results.csv", help="Gdzie zapisać wyniki.")
    ap.add_argument("--top_n", type=int, default=10, help="Ile najlepszych cfg_id wybrać.")
    ap.add_argument("--seeds", type=str, default="0,1,2", help="Seedy ewaluacji (np. '0,1,2').")
    ap.add_argument("--patience", type=int, default=60, help="Patience dla GA.")
    ap.add_argument("--log_every", type=int, default=10, help="Log co ile generacji (wpływa na koszt).")
    ap.add_argument("--rs_trials", type=int, default=200, help="Ile prób random_search na każdy seed/przypadek.")
    ap.add_argument("--rs_presence", type=float, default=0.7, help="prob_presence dla random_search.")
    ap.add_argument("--easy_wh", type=str, default="50,50,50", help="Easy warehouse Wx,Wy,Wz.")
    ap.add_argument("--mid_wh", type=str, default="35,35,35", help="Medium warehouse Wx,Wy,Wz.")
    ap.add_argument("--hard_wh", type=str, default="20,20,20", help="Hard warehouse Wx,Wy,Wz.")
    args = ap.parse_args()
    ...
    for rank_i, cfg_row in top_cfgs.reset_index(drop=True).iterrows():
        ...
        for wh_name, wh in warehouses:
            for seed in seeds:
                ...
                ga_util = ga_score / warehouse_volume(wh) if warehouse_volume(wh) > 0 else 0.0
                ...
                rs_util = rs_score / warehouse_volume(wh) if warehouse_volume(wh) > 0 else 0.0
                ...
                rows.append({
                    ...
                    # meta (z rankingu)
                    "rank_score_mean": to_float(cfg_row.get("rank_score_mean", 0.0)),
